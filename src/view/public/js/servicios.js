...
async function cargarEstadisticasServicios() {
  try {
    ...
    if (res.ok) {
      ...
      const error = await res.json();
      alert("Error al cargar estadísticas: " + (error.error || error.message));
    }
  } catch (err) {
    ...
    <div class="estadisticas-generales">
      ...
      <div class="stats-grid">
        <div class="stat-item">
          ...
          <span class="stat-value">${generales.total_servicios}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">Activos:</span>
          <span class="stat-value">${generales.servicios_activos}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">Inactivos:</span>
          <span class="stat-value">${generales.servicios_inactivos}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">Categorías:</span>
          <span class="stat-value">${generales.total_categorias}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">Precio Promedio:</span>
          <span class="stat-value">Q${parseFloat(generales.precio_promedio).toFixed(2)}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">Tiempo Promedio:</span>
          <span class="stat-value">${Math.round(generales.tiempo_promedio_minutos)} min</span>
        </div>
      </div>
    </div>
  ...
  por_categoria.forEach(cat => {
    html += `
      <tr>
        ...
        <td>${cat.total}</td>
        <td>${cat.activos}</td>
        <td>Q${parseFloat(cat.precio_promedio_categoria).toFixed(2)}</td>
      </tr>
    `;
  });
